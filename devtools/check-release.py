...
#
# check-release.py -- script to verify a libpaf release
#
...
def hdr_get_version(version_name, version_re, commit):
    hdrobj = commit.tree / 'include/paf.h'
    hdr = hdrobj.data_stream.read().decode('utf-8')
    m = version_re.search(hdr)
    if m is None:
        fail("%s version not found in <paf.h>." % version_name)
...
def check_meta(repo, release_commit):
    ...
    if tag_impl_version != impl_version:
        fail("Version according to tag and according to <paf.h> differ")
    ...
    print("  API version (from <paf.h>): %s" % api_version)
    print("  Implementation version (from <paf.h>): %s" % impl_version)
...
def check_abi(repo, release_commit):
    ...
    so_file = "%s/libpaf-%s/.libs/libpaf.so" % (temp_dir.name, version)
    prev_so_file = "%s/libpaf-%s/.libs/libpaf.so" % \
        (prev_temp_dir.name, prev_version)

    hdr_dir = "%s/libpaf-%s/include" % (temp_dir.name, version)
    prev_hdr_dir = "%s/libpaf-%s/include" % (prev_temp_dir.name, prev_version)
